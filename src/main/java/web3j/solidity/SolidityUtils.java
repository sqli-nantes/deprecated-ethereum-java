...
public abstract class SolidityUtils {
    ...
    public static String removeHexIdentifier(String hex){
        ...
        if( index == 0 ) return hex.substring(index+2);
        return hex;
    }
    ...
    * By M.PLOUHINEC
    ...
    public static String bigDecimalToHexString(BigDecimal value){
        ...
        if(mod4 == 0) scale = value.scale();
        else scale = value.scale() + (4-mod4);

        factor = new BigDecimal(BigInteger.valueOf(2).pow(scale));

        if(value.signum() >=0) base = value.multiply(factor);
        else base = value.multiply(factor).multiply(BigDecimal.valueOf(-1));
    ...
    public static boolean isAddress(String value){
        ...
        if( isallCaps || isallLowers ) return true;
    ...
    public static String toChecksumAddress(String address){
        ...
        for(int i=0;i<address.length();i++){
            ...
            if( hashI > 7) checkSumAddress += addressI.toUpperCase();
            else checkSumAddress += addressI;
        }
        return checkSumAddress;
    }

    public static String toAddress(String value){
        if( isStrictAddress(value) ) return value;
        Pattern p = Pattern.compile("^[0-9a-f]{40}$");
        boolean isAddress = p.matcher(value).matches();
        if( isAddress ) return "0x"+value;
